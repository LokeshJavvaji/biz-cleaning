...
return (
  <div className="flex flex-col gap-4 p-6 container mx-auto">
    <p className="mt-4 text-4xl font-bold text-center" style={{ color: 'red' }}>IMPORTANCE OF CLEANING</p>

    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:justify-center gap-10">
      {techTips.map((cardData, index) => (
        <TechnologyCard key={index} data={cardData} />
      ))}
    </div>
    <ul className="text-xl list-inside text-center">
